...
<?php $site_id = $this->session->userdata('siteID'); ?>
...
<script>
	...
	function getPetTales(sort,taleLocation,keyword) {
		...
		jQuery.ajax({
		  ...
		  data: { sort:  sort, taleLocation: taleLocation, keyword: keyword }
			...
			if(json) {
				...
				for (var key in tales) {
					...
					if(key>15){
						...
						var pateTaleDisplay='visible';
					}
					
					if(t['story_pet_passdate']!='0000-00-00') {
						var talePassDateArray = t['story_pet_passdate'].split("-");
						var talePassDateString = new Date(talePassDateArray[0], talePassDateArray[1]-1, talePassDateArray[2], 0, 0, 0, 0);
					...
					if(t['story_pet_birthday']!='0000-00-00') {
						...
						var taleBirthDateString = new Date(taleBirthDateArray[0], taleBirthDateArray[1]-1, taleBirthDateArray[2], 0, 0, 0, 0);
			...
			jQuery("#pet_tales_list").find('img').each(function(){
				    ...
				    complete: function(xhr, stat)
				    {
				    	if(xhr.getResponseHeader('Content-Type') != 'image/jpeg' ){
							...
							jQuery.ajax({
							    ...
							    {
							    	if(xhr.getResponseHeader('Content-Type') != 'image/jpeg' ){
							     		src = "<?php echo $this->config->item('defaultStoryImage');?>";
							     		image.attr('src',src);
							     	}
							    }
							});
	...
	function loadImage(x){
		...
		if(currentPetTaleName.length==0){
			currentPetTaleLinkName = "No Pet Tales Found";
			currentPetTaleName = "No Pet Tales Found";
			jQuery("#featured_pet_tale_link").hide();
		}
		       		...
		       		<li>
		       			...
		       			<select name="pet_tale_location_select" class="pet_tale_location_select" id="pet_tale_location_select">
		       				...
		       				<?php foreach($locations as $l) {?>
		       					<?php 
		       						if($site_id == $l['site_id'])
		       							$locationCheck = 'selected="selected"';
		       						else
		       							$locationCheck = '';
		       						
		       						echo "<option value='{$l['site_id']}' $locationCheck>{$l['name']}</option>";
		       					?>
		       				<?php } ?>
		       			</select>
		       		</li>
